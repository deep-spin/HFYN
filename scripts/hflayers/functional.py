...
def normalize_tensor(tensor, dim=-1, eps=1e-6):
    # Compute the norm along the last dimension
    norms = torch.norm(tensor, dim=dim, keepdim=True)
    
    # Mask the elements where norm is less than epsilon
    mask = norms > eps
    masked_norms = torch.where(mask, norms, torch.ones_like(norms))
    
    # Divide each element by its norm (where norm > eps)
    result = tensor / masked_norms
    
    return result

def has_zero(tensor):
    return np.any(tensor == 0, axis=-1)
def entmax(Z, alpha, dim=-1):
    alpha = alpha.unsqueeze(1).unsqueeze(1) 
    if alpha.requires_grad:
        alpha = torch.clamp(alpha, min=1.01, max=2)
        return entmax_bisect(Z, alpha, dim=dim)
    else:
        condition1 = alpha == 1.0
        condition2 = alpha == 1.5
        condition3 = alpha == 2.0

        result = torch.where(condition1, torch.softmax(Z, dim=dim),
                torch.where(condition2, entmax15(Z, dim=dim),
                    torch.where(condition3, sparsemax(Z, dim=dim),
                        entmax_bisect(Z, alpha, dim=dim))))
        return result



def hopfield_core_forward(ypsi,
                          query,                           # type: Tensor
                          key,                             # type: Tensor
                          value,                           # type: Tensor
                          alpha,
    ...
    while update_active_heads.any():
        ...
        if update_step == 0:
            if not use_separate_proj_weight:
                ...
                if ypsi=="layer_normalization":
                    k = query_ln(k.reshape(k.shape[:2], num_heads, int(k.shape[-1]/num_heads))).reshape(k.shape)
                    v = query_ln(k.reshape(k.shape[:2], num_heads, int(k.shape[-1]/num_heads))).reshape(k.shape)
                    q = query_ln(k.reshape(k.shape[:2], num_heads, int(k.shape[-1]/num_heads))).reshape(q.shape)
            else:     
                _start, _end = 0, hopfield_dim
                if query_as_static:
                    q = query.repeat(1, num_heads, 1)
                ...
                if ypsi=="layer_normalization":
                    k = query_ln(k.reshape(k.shape[0], k.shape[1], num_heads, int(k.shape[-1]/num_heads))).reshape(k.shape)
                    v = query_ln(v.reshape(k.shape[0], k.shape[1], num_heads, int(k.shape[-1]/num_heads))).reshape(k.shape)
                    q = query_ln(q.reshape(q.shape[0], q.shape[1], num_heads, int(q.shape[-1]/num_heads))).reshape(q.shape)
                elif ypsi=="normalization":
                    k = normalize_tensor(k.reshape(k.shape[0], k.shape[1], num_heads, int(k.shape[-1]/num_heads))).reshape(k.shape)
                    v = normalize_tensor(v.reshape(k.shape[0], k.shape[1], num_heads, int(k.shape[-1]/num_heads))).reshape(k.shape)
                    q = normalize_tensor(q.reshape(q.shape[0], q.shape[1], num_heads, int(q.shape[-1]/num_heads))).reshape(q.shape)
    ...
    if ypsi == "identity":
        ...
        attn_output = normalize_tensor(attn_output)
    elif ypsi == "layer_normalization":
        attn_output = query_ln(attn_output)
    else:

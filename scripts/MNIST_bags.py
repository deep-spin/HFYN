...
def get_args():
    ...
    parser.add_argument('--k_sparsemap', help='k exact ones for sparsemap', default=2, type=int)
    ...
    parser.add_argument('--k_data', help='k data', default=1, type=int)
    parser.add_argument('--alpha', help='alpha for normmax', default=5, type=int)
    parser.add_argument('--mean', help='mean of bags size', default=10, type=int)
    parser.add_argument('--var', help='var of bags size', default=1, type=int)
...
class HfPooling(Module):    
    def __init__(self, alpha = 1.0, normmax=False, sparseMAP = False, k=2, beta = 1.0, hidden_size = 32, alpha_as_static = True, n_heads = 1, dropout = 0.0, normalize_hopfield_space = False):
        ...
        alpha_as_static=alpha_as_static, dropout=dropout, normalize_hopfield_space=normalize_hopfield_space, ypsi="identity")
        self.dp = Dropout(
            p=0.1
        )
...
if __name__ == '__main__':
    ...
    if args.method == "sparsemap":
        csv_filename = f"results_MNIST_budget/MNIST_k_{args.k_data}_{args.method}_k_{args.k_sparsemap}_trainsize_{args.train_size}.csv"
    elif args.method == "normmax":
        csv_filename = f"results_MNIST_budget/MNIST_k_{args.k_data}_{args.method}_alpha_{args.alpha}_trainsize_{args.train_size}.csv"
    else:
        csv_filename = f"results_MNIST_budget/MNIST_k_{args.k_data}_{args.method}_trainsize_{args.train_size}.csv"

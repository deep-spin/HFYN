...
class HopfieldCore(Module):
    ...
    def __init__(self,
                 ...
                 alpha_as_static=True,             # type: bool
        ...
        self.ypsi = ypsi
        self.k = k
        self.alpha_as_static = alpha_as_static
        if not alpha_as_static:
        ...
        if self.ypsi == "layer_normalization":
            self.query_ln = nn.LayerNorm(
                normalized_shape=head_dim, elementwise_affine=True,
                eps=True)
        else:
            self.register_parameter('query_ln', None)
    ...
    def forward(self,
        ...
        if not self._qkv_same_embed_dim:
            ...
            return hopfield_core_forward(ypsi=self.ypsi,
                query=query, key=key, value=value, alpha = self.alpha, normmax=self.normmax, sparsemap = self.sparsemap, k_ones = self.k, embed_dim_to_check=embed_dim_to_check, num_heads=self.num_heads,
                ...
                v_proj_weight=self.v_proj_weight, query_ln=self.query_ln,

...
class HopfieldMIL(nn.Module):
    def __init__(self, feat_dim, emb_dims, emb_layers, num_heads, hidden_size, beta, dropout, alpha, k, sparseMAP = False, normmax = False):
        ...
        alpha_as_static=True, dropout=dropout, ypsi=args.ypsi, pattern_projection_as_connected=True)
...
if __name__ == '__main__':
    ...
    args = get_args()
    if args.mode == "sparsemap":
        csv_filename = f"results_pure_hop_n/{args.dataset}_{args.mode}_k_{args.k}_ypsi_{args.ypsi}.csv"
    elif args.mode == "normmax":
        csv_filename = f"results_pure_hop_n/{args.dataset}_{args.mode}_alpha_{args.alpha}_ypsi_{args.ypsi}.csv"
    else:
        csv_filename = f"results_pure_hop_n/{args.dataset}_{args.mode}_ypsi_{args.ypsi}.csv"
    main(args)